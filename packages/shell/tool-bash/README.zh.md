...
- **请求只来自命名参数。** 工具从不暴露 `stdin`、`env` 或 `stdoutMaxBytes`；它只用命令／workdir／超时／信号字段、注册表收集的 `dshEnv`，以及挂载该注册表时来自 `ctx.shellExecEnv` 的可信快照构建每个请求，因此模型提供的键无法替换可信值。
